...
@
\caption{Percentage of populations with $x$ number of Females at death/birth step $t$.}
\end{figure}
...
In out model the transition matrix has two absorbing states. Then it can be put into the following canonical form:
\begin{align*}
M = \begin{pmatrix}
Q & R\\ 
O & I_2
...
We calculated the expected survive time by using different $k$ values (2~15) :
<<eval=FALSE>>=
result=zeros(15,1);
for k=2:15
