...
\subsection*{Fundamental matrix and expected surviving year}
In out model the transition matrix has two obsorbing states. Then it can be put into the following canonical form:
\begin{align*}
M = \begin{pmatrix}
Q & R\\ 
O & I_2
...
where $I_{2k-1}$ is the identity matrix. the entry of the fundamental matrix $F$ is the expected number of times the Markov chain is in stat $j$, given that the chain started in state $i$.  In this case the expected number of steps before being absorbed when starting in transient state I is the ith entry of the vector:
\begin{align*}
S=F1
\end{align*}
...
The expected step number is $1168$ and then we can calculate the expected surviving year is $9349$.

We calculated the expected survive year by using different $k$ values (2~15) :
<<eval=FALSE>>=
result=zeros(15,1);
for k=2:15
