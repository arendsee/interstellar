...
possible (see Appendix), but is complicated. Here we will simulate small
...
population size, the excess babies are tossed out the airlock. 
...
\begin{enumerate}
    ...
    \item Women don't need men to make babies (they have a sperm bank). This
    assumption avoids the complexities of mate choice and inbreeding.
\end{enumerate}
...
produced by a woman will be $20 \times f$.
...
Of interest is the distribution of $T$ as a function of $f$ (fertility), $A$
(maximum age), and $N$ (maximum population size).
