...
Data from simulated populations with varying maximum population ($N$), maximum age ($A$), and fertility rate ($f$) were collected and compared with estimated values from the Markov Model. When not veried, the default values of maximum population, maximum age, and fertility rate were 20, 80, and 0.2 respectively.
...
\caption{Plots of mean and standard deviations across all treatments}
\end{figure}
