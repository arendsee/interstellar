...
with maximum population size. So we can model the distribution of the log transformed population surival time using a linear model of the maximum poplation size.
...
This yields the following $\beta$ values
...
\end{tabular}
\caption{Predicted versus empirical estimates of the mean and standard deviation of two simulations}
\end{table}
...
to have a sufficient probability of survival. To that end we will compare the
2.5 percentiles from the actual data, to the predicted percentiles.
